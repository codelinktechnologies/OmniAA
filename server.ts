...
Return EXACTLY a JSON object matching this schema:
{
  ...
  if (ai) {
    try {
      ...
      console.warn("Failed to generate persona via Gemini, using fallback:", e?.message || e);
      applyFallback();
    }
  } else {
    applyFallback();
...
app.post("/api/setup-telegram-bot", async (req, res) => {
  ...
  try {
    ...
    console.warn("Error setting up Telegram bot:", err);
    return res.json({ 
      success: false, 
...
- If asked to pause or start: trigger toggle_mode or set_status and confirm.`;
  ...
  if (ai) {
    try {
      ...
      console.warn("Gemini failed to generate response for Telegram webhook:", err);
  ...
  try {
    ...
    console.warn("Failed to send message back to Telegram:", err);
  }
});
...
Auto-Agent / Client Acquisition`
  ...
  try {
    ...
    console.warn("Gemini failed to generate custom audit report:", error);
    return res.json({ status: "success", report: fallbackReport, mode: "template" });
  }
});
...
app.post("/api/generate-lead", async (req, res) => {
  ...
  try {
    ...
    console.warn("Gemini failed to generate custom lead:", error);
    return res.json({ status: "success", lead: fallback, mode: "template" });
  }
});
...
app.post("/api/generate-outreach", async (req, res) => {
  ...
  try {
    ...
    console.warn("Gemini failed to generate outreach copy:", error);
    return res.json({ status: "success", content: fallbackOutreach, mode: "template" });
  }
});
...
app.post("/api/heartbeat", async (req, res) => {
  ...
  if (openClawApiKey && openClawEndpoint) {
    try {
      ...
      console.warn("Failed to connect to real OpenClaw.ai endpoint:", err);
    }
  }
  ...
  if (ai) {
    try {
      ...
      console.warn("Failed to generate heartbeat data with Gemini:", err);
    }
  }
